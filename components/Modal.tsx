...
import { selectPost, setModal } from "../slice/post-slice";
...
function Modal() {
  ...
  useEffect(
    () =>
      onSnapshot(doc(firestore, "posts", select.postId), (snapshot) => {
  ...
  const sendComment = async (e) => {
    ...
    await addDoc(collection(firestore, "posts", select.postId, "comments"), {
      comment: comment,
    ...
    router.push(`/${select.postId}`);
  };

  return (
    <Transition.Root show={select.modal} as={Fragment}>
      <Dialog
        as="div"
